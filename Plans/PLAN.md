...
- **Phase 3** — **The Mac app that calls it.** Just a window showing the shuffle — sized to fit, with the pan — that can be taken full screen. No source management, no settings: `pgr_ctl` already does that. This is the milestone that proves the whole idea, and it is small. **As built, 2026-09-12**: the window and full screen, a Settings panel for sources and the wallpaper, and the About box's dashboard link; the pan waits with the saver's motion, and watching and the fit options are not built.
  ...
  - **The agent performance overhaul — in progress since 2026-09-16, in its own plan, `Agent Performance Overhaul.md`.** The app and the screensaver lost the agent three times in one afternoon; a thread sample showed every thread in the shared pool stuck resizing and waiting on the write lock. Built so far: the `TIMING:` and `LOCK:` log lines, a one-second budget on serving's *is it still there?* check, one serial `Resizer` off the pool, a sized request that sends the original when its resize takes over a second, the resize cache returning, eviction after every write instead of on a timer, the refresh locking only to write in pages of 100 read before the lock, and a lane — an actor with a thread of its own — per request. **And the LaunchAgent's `ProcessType` is `Adaptive` since 2026-09-17**: as `Background`, macOS throttled the agent's disk I/O, which was most of a restart's delay and all of serving's long write locks. Still to come: refresh and downloads on actors of their own, and the cache index read from the database at launch so the port does not wait for a walk of the cache.
