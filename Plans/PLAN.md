...
| --- | --- | --- |
| Mac agent | No | `NSPhotoLibraryUsageDescription`. **As built, 2026-09-12**: no entitlements file, no App Group, and no hardened runtime yet; Developer ID and notarization are the 1.0 target. **Since 2026-09-23**: hardened runtime in all three configurations, and `Photos-Go-Round Server.entitlements` carrying `com.apple.security.personal-information.photos-library` — without it a hardened agent is refused PhotoKit |
...
4. ~~**No Xcode project until Phase 3.**~~ **Done: every product is a target in `Photo-Go-Round.xcodeproj` since 2026-09-08**, and the App Group container question it was waiting on went away instead of being settled. See *The agent: registration and permissions*.
...
- **The app** ships in a notarized, stapled DMG. Drag to `/Applications`. **Built 2026-09-23, not yet run:** `Scripts/release-build.sh`, Syd's to run. It archives Release, exports it with Developer ID, checks every bundle for the Developer ID signature and the hardened runtime and the app and agent for the Photos entitlement, then notarizes and staples the app, and then the DMG. It needs a Developer ID Application certificate and a `notarytool` keychain profile, `pgr-notary`, once per Mac; neither existed on 2026-09-23. Xcode's Organizer, *Direct Distribution*, does the same without the DMG, and Syd prefers the DMG. The DMG is plain today; TODO.md, *A finished DMG*, is what it should become.
