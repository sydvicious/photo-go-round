...
The agent is per-user by design, installed in each user's `~/Library/LaunchAgents` with its own database. But it listens on loopback, and every account on the Mac shares loopback. So today anyone logged in can fetch another user's pictures from 9427, and a client that tries the fixed port first would do it by accident. Syd, 2026-09-21: "the danger is that the second user send a request to the first user's agent on the default port."

# Phases

- **Prerequisite — The app installs everything.** Not this plan's work, and it comes first. Syd, 2026-09-21: "We need to implement the app installing everything first, though." It is `TODO.md`'s *Installing by launching the app*, which still needs its own plan.
...
- **Phase 3 — Every client sends it.**
  ...
  - A `401` on the fixed port sends the client to its published one.
...
- **Kept, not remade each launch.** A new one per launch would bring back the gap after a restart that the fixed port closed.
...
- **Which port the second agent binds stays as it is.** It falls back to one from the kernel and publishes it. With the secret in place, a request that reaches the wrong agent is refused, not served.
...
- **Only the agent uses the fixed port today.** Clients read their own user's published `servicePort`, which is why this has not happened by accident yet. `Service Port Plan.md` Phase 2 would change that.
...
- **Deliberately, today:** anyone logged in can `curl http://localhost:9427/…` and get the first user's pictures, their sources, and the dashboard.
- **By accident, once `Service Port Plan.md`'s Phase 2 is built:** the second user's clients try 9427 first, reach the first user's agent, and show the first user's pictures on the second user's desktop and screensaver. Nothing on either side would notice.
- **Not by accident today:** clients still read their own user's published port, and the only thing that uses `ServiceAddress.port` is the agent itself (`RunCommand.swift`).

## Why the app installer comes first

*Claude's reading of Syd's "We need to implement the app installing everything first."* Today the agent, the saver and the wallpaper extension are installed by ⌘R on an `Install …` scheme or by `Scripts/install-*.sh`, from a developer's Xcode. A second user on the same Mac has no way to install their own copy. Until the app installs each user's pieces on that user's first launch, there is no second user to support, and Phase 6 cannot be run.

## Which port the second agent binds

Moved from `Service Port Plan.md` on 2026-09-21. The agent is per-user by design, and two logged-in users each run their own. They cannot both bind 9427 on loopback.
...
- **The fixed port plus the published value.** The second user's agent binds something else and publishes it, and that user's clients find it the way they do now. The first user never pays the cost. This is what the agent already does.

**None of these was ever the danger.** Every option leaves 9427 belonging to whoever logged in first, and any request that reaches it is served. The secret is what fixes that, and with it in place the choice among these three matters much less. The third stays, because it is built.
...
- **A `401` is its own state**, separate from *agent not running*: it means *this is somebody else's agent*, and on the fixed port it sends the client to the published one. From the published port it means the secret changed underneath the client, and the client re-reads it once before reporting.

## What it does not stop

- **A hostile user who takes 9427 first.** If the second user starts a listener on 9427 before the first user's agent, the first user's agent falls back to another port — and any of the first user's clients that try 9427 first send their secret straight to that listener, which can then use it on the real agent. Clients that go to the published port first never do. **So `Service Port Plan.md`'s Phase 2 order is open again**: since its Phase 1, the published port is right the moment the agent starts, and fixed-port-first was for a gap that is already closed. *For Syd to decide.*
...
- A client refused on the fixed port reaches its own agent through the published one.
...
- `Plans/Service Port Plan.md` — the fixed port, and the Phases 2 and 3 this plan reopens.
- `TODO.md`, *Installing by launching the app* — the prerequisite.
...
- `Sources/photogoroundd/Service/HTTPListener.swift`, `Sources/PhotoGoRoundDisplay/PictureClient.swift`, `Sources/PhotoGoRoundDisplay/ServicePort.swift`, `Sources/PhotoGoRoundAgentAPI/Host/Preferences.swift`, `app/mac/Sources/SourceService.swift`, `app/mac/Sources/AboutView.swift`, `Sources/pgr_ctl/InspectCommands.swift`.
