...
The in-memory index does not go away — it is still what answers "where are this photograph's bytes" for the request that is serving one, and it is still what holds the byte counts eviction adds up. What changes is that the *set* of resident photographs is now also queryable, and `cached_at` has three readers: the deck's pool predicate, the eviction order's tiebreak for a photograph that has never been shown, and the refresher's stop-condition count. (The deck's reader went on 2026-09-05 with *Deal over everything* and came back narrower on 2026-09-07: the pool predicate reads it only for a source marked unavailable, which deals what is held and nothing else. See `Missing Albums Plan.md`.) Note which one is *not* on that list — the refresher's draw itself, which asks the in-memory index whether it already holds what it picked rather than filtering the query. The column is for the questions that need a count or an order.
...
- `MacOS/Shared/Sources/PhotoGoRoundKit/Deck/Deck.swift`, `Deck+Consumers.swift` — the shuffle, the pass, the window, the claim.
- `MacOS/Shared/Sources/PhotoGoRoundKit/Cache/PhotoCache.swift`, `PhotoQueue.swift`, `CacheQueue.swift`, `QueueFiller.swift`, `PhotoStore.swift` — everything this plan rearranges.
- `MacOS/Agent/Sources/RunCommand.swift` — `FillerBox`, the gauge, the bridge.
- `MacOS/Agent/Endpoints/Sources/PictureEndpoint.swift` — `queueRanShort` and `queueCameUpEmpty`.
