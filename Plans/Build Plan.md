...
Five products — the app, the wallpaper extension, the screensaver, the agent, and `pgr_ctl` — each built by its own Xcode target and installed by its own build phase, with one script per product and one script that runs them all for CI.

# Rationale

The products are built four different ways today: the app and the wallpaper extension from Xcode, the screensaver from a script that drives `xcodebuild`, the agent from a script that calls `swift build` rather than its own target, and `pgr_ctl` from `swift run`. Installing is a fifth thing again — a copy for the saver, a plist for the agent, nothing for the app, and for the extension a rule nobody has written down. That is four mental models for one project, none of them exercised by CI, and it is why a wallpaper extension gets registered every time the app is run whether or not anything in it changed. Syd, 2026-09-15: "My vision is that the target for each thing would have a build phase that would install it for dev, and for release, and different schemes would call that correctly."

# Phases

- *The targets* — every product builds independently from its own scheme.
  - The agent moves from `swift build` to its `Photo-Go-Round Server` target.
  - `pgr_ctl` likewise, from `swift run` to its own target.
...
- *The install phases* — each product has an install target of its own. **Three of five built 2026-09-15.**
  - **`Install Wallpaper Extension`**, **`Install Agent`** and **`Install Screen Saver`** — each an aggregate target depending on the product it installs, with `ENABLE_USER_SCRIPT_SANDBOXING = NO` and a script in `Scripts/`.
  - Still to do: the app and `pgr_ctl`, neither of which has a dev install worth the name yet.
  - Dev installs in place from the build directory; release leaves it to packaging.
  - **Uninstall is `Scripts/uninstall.sh`**, run by hand.
- *The scripts* — one per product, same options, same defaults.
  - `make-app.sh`, `make-wallpaper-extension.sh`, `make-saver-bundle.sh`, `make-agent-bundle.sh`, `make-pgr-ctl.sh`.
  - Each takes `--output`, `--release`, `--install`, `-h`, and defaults its output to DerivedData.
...
- **Installing is a build phase, not a script's job.** The scripts call the same phases the schemes do, so CI and Xcode cannot drift apart. Syd, 2026-09-15. *Under question since 2026-09-16: Syd, "we should think about separating build and install for everything, so command-b builds and command-r runs" — `TODO.md`, not decided.*
- **An install runs from a target of its own, not from a phase on the product's target.** A Run Script phase always runs *before* its own target is code-signed, and `pkd` refuses an appex inside an unsigned bundle — "plug-ins outside containing apps must be protected by SIP". So `Install Wallpaper Extension` is an aggregate target depending on `Photo-Go-Round Wallpaper Host`, which is signed by the time the script runs. Syd, 2026-09-15: "separate install target". Measured the same day, after the phase-on-the-target version failed exactly this way.
...
- **Every install checks that the agent has Photos access.** Syd, 2026-09-15, of the saver's install: "it also needs to make sure the agent has photos access." `Scripts/ensure-photos-access.sh` holds it once and all three installs call it: find the published port, read the status, and ask only when nobody has decided. It never fails an install — an agent that is not running, or an owner who said no, is reported and the bundle still lands.
...
- **No third-party build tooling.** `xcodebuild`, `swift build`, `codesign`, `pluginkit`, `launchctl`, and shell. Nothing else.
...
- `Scripts/make-saver-bundle.sh` and `Scripts/make-agent-bundle.sh` already exist and are the model the rest follow — options, an `--install` that is the owner's call, and output under DerivedData.
...
- **Not established:** whether a rebuilt agent is asked again. TCC records a grant against the code signature, and a DerivedData build is re-signed on every build, so the grant may or may not survive. Worth watching rather than assuming; `make-agent-bundle.sh --install-to ~/Applications` with a stable identity is the route that avoids the question.

## What "install" means, per product

Each of these is a decision to take on its own, and none is taken yet.

- **The app.** Nothing, in dev: it runs from Xcode. Release is a signed, notarized copy in `/Applications`.
- **The wallpaper extension.** Dev: the `Photo-Go-Round Wallpaper Host` target embeds it, so building that scheme produces a registrable bundle; the install phase runs `pluginkit -r` on whatever holds the identifier, then `pluginkit -a` on the host's embedded appex, straight from DerivedData. *Since 2026-09-15 only registrations whose bundle is gone are removed, and since 2026-09-16 also those whose bundle now holds a different identifier; the identifier is read from the appex.* Nothing is copied and nothing is launched. Release: it ships inside the app wrapper and the app installs it. **Decided 2026-09-15**, Syd: "for dev, we can make a shell app to put it in". **Measured working the same day**: the host built, carried the appex, registered from DerivedData with nothing running, and unregistered again cleanly.
- **The screensaver.** Dev: `Install Screen Saver` depends on `Photo-Go-Round Saver` and runs `Scripts/install-saver.sh`, which copies the bundle into `~/Library/Screen Savers` and stops `legacyScreenSaver` and `ScreenSaverEngine` — both cache a loaded bundle for the life of the process, so without the kill a rebuild runs the previous build and looks like a change that did nothing. Built 2026-09-15. `make-saver-bundle.sh --install` keeps its own copy of these steps, for a machine with no Xcode project open. A symlink instead of a copy is still unmeasured.
- **The agent.** Dev: **full install**, decided 2026-09-15 — Syd chose it over building the bundle alone and over writing the plist without starting it. `Install Agent` depends on `Photo-Go-Round Server`, and its script boots the job out, writes `~/Library/LaunchAgents/com.sydpolk.photogoround.server.plist` pointing at the built binary, and boots it back in. Syd, 2026-09-10: "whatever launchctl needs should be put into ~/Library/LaunchAgents so multiple users don't clobber each other."
  ...
  - **The plist points into DerivedData**, which is right for development and wrong for anything left running — a clean build directory takes the agent with it. `Scripts/make-agent-bundle.sh --install-to ~/Applications` stays the stable route.
...
- **`TODO.md`, *Build products out of the repo*.** `make-saver-bundle.sh` defaults to `$REPO/build/xcode` and `make-agent-bundle.sh` to `$REPO/build`, with `swift build` into `.build`. This plan moves every default to DerivedData; the `.build` development library stays where it is, and that item's real question — where development storage lives once no build writes into the repo — is untouched.
...
- `Scripts/make-saver-bundle.sh`, `Scripts/make-agent-bundle.sh`, `Scripts/make-wallpaper-extension-probe.sh`, `Scripts/photogoroundd`, `Scripts/scrub-dev.sh`.
