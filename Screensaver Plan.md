...
- **Phase 3 — the saver shows photographs.** The real target: a `ScreenSaverView` hosting a `PictureLayerView`, driven by `Shuffle`, at ten seconds a picture.
  - `startAnimation()` and `stopAnimation()` start and stop the loop; `animateOneFrame()` is never implemented. **Both are called repeatedly in one host process**, so starting is idempotent and stopping tears the loop down — observed in Phase 1, and a leak with a schedule attached if it is missed.
  - One instance per display, each its own consumer, each asking at its own pixel size.
  - A picture already showing is never taken down — inherited from `Shuffle`, and the rule that matters most here. It has to survive a stop and start too, or every wake is a black frame.
  - **Exit gate:** it is the screensaver on the machine for an evening, and it is still showing photographs in the morning.
...
- **The preview thumbnail:** shipped placeholder, static card, a new `/v1/peek`, or one card per settings visit. My preference is the placeholder for v1 — and Phase 1 raises the possibility that there is nothing to decide, since no preview instance was ever observed.
...
- **The empty state and burn-in:** static words, words repositioned once per dwell, or the bouncing treatment after all. My preference is repositioning once per dwell, as the smallest thing that is not a hazard.
...
**Applied 2026-09-07, at Syd's instruction, and annotated rather than overwritten** — `PLAN.md`'s own house style keeps a wrong argument visible when the error is easy to make again, so each of these is a marked correction beside the original text rather than a replacement of it. One typo was corrected in passing and flagged in place: the sandbox section's closing line said the spike "determines the shape of Phase 5", meaning Phase 6.

The six, as they stood:
...
- `app/saver/Sources/PGRScreenSaverView.swift`, `app/saver/Info.plist`, and `Scripts/make-saver-bundle.sh` — Phase 1's stub and how it is assembled and installed.
