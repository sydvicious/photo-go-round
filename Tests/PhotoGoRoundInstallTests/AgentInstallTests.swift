...
struct AgentInstallTests {
    ...
    private let bundle = URL(filePath: "/build/Products/Debug/Photo-Go-Round Server.app")
    ...
    private func surroundings(
        ...
        label: String? = "com.sydpolk.photogoround.server.debug",
        loaded: Bool = false,
    ...
    func buildDirectoryIsNoticed() throws {
        let derived = try AgentInstall.plan(
            for: URL(filePath: "/Users/x/Library/Developer/Xcode/DerivedData/P-abc/Build/Products/Debug/Photo-Go-Round Server.app"),
            launchAgents: agents, surroundings: surroundings())
        #expect(derived.pointsIntoBuildDirectory)

        let installed = try AgentInstall.plan(
            for: URL(filePath: "/Applications/Photo-Go-Round Server.app"),
            launchAgents: agents, surroundings: surroundings())
    ...
    func jobDescriptionShape() throws {
        let job = JobDescription(
            label: "com.sydpolk.photogoround.server.debug", program: binary)
        let decoded =
            try PropertyListSerialization.propertyList(from: job.encodedPlist(), format: nil)
            as? [String: Any]
        let values = try #require(decoded)

        #expect(values["Label"] as? String == "com.sydpolk.photogoround.server.debug")
