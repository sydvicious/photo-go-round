...
N_RING = 40                # cards in the ring besides the house card
SPACING = 264 / N_RING     # same arc as the dozen-card version, packed tighter
...
# Mixed card colours: not another solid blue icon, and still lively in
# red-green colour blindness, where they read as blues and golds.
PALETTE = ["#e8674f", "#f2a93b", "#2fb3a1", "#8a6fd8", "#e86aa6"]
...
def ordinary_card(phi, i):
    ...
    if shows_face:
        ...
        body = mix("#ffffff", PALETTE[i % len(PALETTE)], 0.45 + 0.55 * t)   # paler toward the back
...
def showing_card():
    # Bold shapes, no small details (the window is gone): at 32 points the
    # picture is a few dozen pixels across.
    # Dark roof and tree against light walls and grass: the picture has to read
    # by lightness as well as hue, for red-green colour blindness.
    ...
    m = affine(pts, w, h)
    rays = ''.join(f'<line x1="0" y1="0" x2="{30*math.cos(a):.1f}" y2="{30*math.sin(a):.1f}" stroke="#ffb800" stroke-width="9" stroke-linecap="round"/>' for a in [k*math.pi/4 for k in range(8)])
    art = f'''
      <rect width="{w}" height="{h}" rx="14" fill="#ffffff"/>
      <clipPath id="pic"><rect x="12" y="12" width="{w-24}" height="{h-24}" rx="8"/></clipPath>
      <g clip-path="url(#pic)">
        <rect width="{w}" height="{h}" fill="url(#sky)"/>
        <rect y="145" width="{w}" height="{h}" fill="#9be27a"/>
        <g transform="translate(58 52)">{rays}<circle r="24" fill="#ffd21f"/></g>
        <rect x="110" y="100" width="90" height="50" fill="#ffe68f"/>
        <polygon points="100,102 155,62 210,102" fill="#a3271c"/>
        <rect x="145" y="118" width="20" height="32" fill="#34246e"/>
        <rect x="241" y="108" width="11" height="42" fill="#5a3a1e"/>
        <circle cx="246" cy="94" r="32" fill="#1d6b36"/>
      </g>
      <!-- Thin navy edge: the white border alone vanishes against the pale yellow. -->
      <rect x="2" y="2" width="{w-4}" height="{h-4}" rx="13" fill="none" stroke="#2f4a73" stroke-width="6"/>'''
...
cards = [ordinary_card(-SPACING * k, k) for k in range(1, N_RING + 1)]
...
open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "Mockups", "ring.svg"), "w").write(svg)
