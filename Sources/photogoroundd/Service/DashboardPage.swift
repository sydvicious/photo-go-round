...
///
/// **In `Sources/photogoroundd/js/`, not in a Swift string.** They were one raw
...
/// get a real editor and `node --check`. `Package.swift` excludes the folder, so
/// SwiftPM does not treat them as resources of its own.
///
/// **Where they are found, in order:**
///
/// 1. **The agent's app bundle**, `Contents/Resources`. The Xcode target
///    `Photo-Go-Round Server` syncs the whole `photogoroundd` folder and copies
///    these there, so an installed agent — from DerivedData or shipped — reads
///    its own copy.
/// 2. **`js/` in the agent's source folder**, for a build with no app bundle:
...
enum DashboardPage {
    ...
    /// `Sources/photogoroundd/js/`, reached from
    /// `Sources/photogoroundd/Service/DashboardPage.swift`.
    static func sourceDirectory(source: String = #filePath) -> URL {
        URL(filePath: source)
            .deletingLastPathComponent()  // Service
            .deletingLastPathComponent()  // photogoroundd
            .appending(path: "js", directoryHint: .isDirectory)
    }
}
