...
- **Every photo is dealt; whether it is *shown* turns on its bytes being local when its turn comes.** An unmounted drive stops referenced photos being shown immediately; a vanished Photos library does not, since those were materialized into our cache. Orphans then shuffle out gradually as FIFO evicts them — the cache's ordinary behavior is the garbage collector. **Since 2026-09-07 an unavailable source's unheld photographs are not dealt at all** — see the amendment two bullets up — so an unmounted drive's referenced photographs leave the pool rather than being dealt and dropped.
...
**Superseded 2026-08-26 by *Deck and Queue v2.md*, and revised again 2026-09-06.** Eviction is strictly least-recently-*used*, over photographs rather than `(photo, resolution)` entries, and nothing is exempt. The FIFO-by-write-time policy below, and the queued-photograph protection that went with it, are both gone.
...
- **The byte ceiling is the primary control, and the only one.** A thousand photos is somewhere between 2 GB and 100 GB depending on whether they are phone JPEGs or ProRAW, so a count never bounded the thing that mattered. It began life as a safety valve behind a count cap; it is now the whole policy.
...
The same policy with a different filling schedule. **The smaller numbers are gone: since 2026-09-06 there is one `CacheSettings.default` for every platform**, and the `phone` preset — a 2 GB ceiling — went with the Mac's drop to 1 GB, which had made the phone's the larger of the two. A ceiling that follows the queue does not vary by device, because the queue does not; a device that genuinely needs a different number is told one through the preference. The queue is still shorter on a phone. Filling happens in the foreground and in `BGProcessingTask` windows when charging — and because one request yields one finished picture, a task that runs out of time simply stops asking. Nothing is ever half-done, so there is no boundary to resume from.
...
**One consequence to hold on to, because the plan promises otherwise elsewhere.** *Always show something, even a repeat* says a walk that finds nothing servable deals from what is already cached. At 10 GB that fallback pool was thousands of photographs; at 1 GB it is a few hundred. An offline session is now much more repetitive than that section implies. Accepted rather than solved.
...
- `fitting:` through `serve`, `bytesHere`, `firstQueuedWithBytes`, `waitForBytes` — the box existed so an evicted original could be answered from a rendering at exactly that size. `bytesHere` is now `residentURL`, which still resolves a referenced file in place.
...
**Of that list, only the fit and the black bars were built.** `AspectFit` is in `PhotoGoRoundDisplay` and the window uses it; `Pan` is in there beside it, tested, with no callers anywhere in the project; the cross-fade and the bouncing empty state were never started. The rehearsal did not happen, and Phase 6 arrived at the sandbox with the visual design still ahead of it rather than behind it. **The argument above is not wrong and is worth keeping** — it is why the 2026-09-07 decision splits the saver's v1 into the picture now and the motion later, in the window, where this section always said it belonged.
...
**Planned in `Wallpaper Plan.md`, 2026-09-10, which settles several points in this section.** The wallpaper is a client — the Mac app runs it first, then a binary of its own — so wherever this section says *the agent* re-applies or reasserts, read *whatever runs the wallpaper*. The background colour is not ours to set: the space around the photograph is the fill colour chosen in System Settings. The files the desktop points at live in `~/Library/Application Support/com.sydpolk.photogoround.wallpaper.{dev|prod}/`, never in the agent's container — "the app should not need to see the agent's container." And it runs only while the app's *Also set wallpapers* checkbox is ticked.
...
| Mac config app | No | Same; ~~registers the agent via `SMAppService`~~ installs a per-user LaunchAgent plist in `~/Library/LaunchAgents` (2026-09-10). Probably ships as a menu-bar app rather than the full window — TODO.md, *A menu-bar app for shipping* |
