...
- **Phase 1** — **The Mac server for pictures.** `PhotoGoRoundKit` — SQLite schema and migrator, deck, hands and consumers, source protocol, chunked ingestion, cache — plus the headless macOS process that owns it. An `LSUIElement` bundle registered via `SMAppService`. Folder and explicitly-selected-file sources. No UI at all.
- **Phase 2** — **`pgr`, an internal Swift command-line tool to exercise the server.** Add sources, scan, peek at the deck, deal hands, inspect the cache, run the shuffle-quality statistics. This is how the server is proven correct before any window exists, and it stays the rig for everything scriptable afterward. Never shipped.
- **Phase 3** — **The Mac app that calls it.** Just a window showing the shuffle — sized to fit, with the pan — that can be taken full screen. No source management, no settings: `pgr` already does that. This is the milestone that proves the whole idea, and it is small.
...
- **Phase 5** — iOS widget: WidgetKit extension sharing an App Group container, advancing the deck from the timeline provider.
...
- **Ingestion is chunked: 10 immediately, then 10 at a time.** The first ten make the system usable within seconds of adding a source; the rest fill in behind them without saturating PhotoKit, the network, or the disk.
...
- **The cache is clearable on demand, and guarded against a full disk.** `pgr cache clear`, optionally per source or restricted to unavailable ones, never touching deck history. Separately, free space is checked before every chunk, because a photo count cannot bound bytes.
...
- **`pgr` and the Mac app are permanent test harnesses, not scaffolding.** The command line covers anything scriptable, statistical, or concurrent; the app covers anything visual or timing-dependent. Every later surface is exercised through one of them before it gets its own home.
- **The Mac app is just a window that can go full screen.** No source management, no settings — `pgr` owns those. A full-screen window is visually what the screensaver will be, so the display behavior gets designed there and Phase 6 is left with only the sandbox to solve.
...
- **The database holds state; `UserDefaults` holds preferences.** Sources, deck, hands, and cache are state. Fits, timings, transitions, and caps are preferences, living in the App Group suite so they are settable from the command line.
...
- *A migrator.* A `user_version` pragma, an ordered array of migration closures, and a loop that applies the ones above the current version inside a transaction. This is genuinely about 40 lines. It is also the single place where a hand-rolled layer most often goes wrong later, so it gets tests from the first commit: apply from empty, apply from every intermediate version, and assert the resulting schema matches a freshly-created one.
...
A photo that is no longer in its source stops being dealt, immediately. It leaves the queue, it leaves any outstanding hand, and its cached bytes and derivatives are deleted.

Leaving the queue is not the same as leaving the database. The row stays, with `available = 0`; every query that feeds a deal already filters on `available`, so a vanished photo is unreachable the moment the flag flips. Keeping the row costs 200 bytes and buys three things:

- **Unmounted volumes are not deletions.** An external drive that is not plugged in makes ten thousand photos vanish at once. Soft-deleting means plugging it back in restores them — with their deal history intact, so they resume their place in the rotation rather than all becoming immediately eligible at once. Hard-deleting would mean re-enumerating and re-downloading everything every time you undock. The scanner should additionally refuse to mark anything unavailable when the *volume itself* is missing, rather than processing ten thousand individual disappearances.
- **It is what makes rename-and-move tracking possible later.** Recognizing that a file which just appeared at a new path is the same file that vanished from an old one requires the old row to still exist. Hard-deleting on disappearance forecloses the feature below.
- **It keeps history honest.** "This photo has been shown 40 times" survives a temporary absence.

Detection is by rescan rather than by watching. A periodic scan diffs the source and flips flags in both directions — vanished photos to unavailable, returned photos back. On the Mac an `FSEventStream` on folder sources makes this near-immediate rather than waiting for the next scheduled pass, and `PHPhotoLibraryChangeObserver` does the same for Photos sources once that provider lands. Neither is a substitute for the periodic scan, which remains the backstop for changes made while the agent was not running.

There is also a fast path that costs nothing: a deal that finds its file missing marks the photo unavailable on the spot and deals again. A photo deleted thirty seconds ago should not produce a blank screen while waiting for the next scheduled scan.
...
**Does FSEvents make this unnecessary?** Partly, and it is worth being precise about how far it goes, because it looks like a complete answer and is not.

What FSEvents genuinely gives us, once we are already watching directories for the preferences plist and for folder-source rescans:

- **Rename and move events, live.** With `kFSEventStreamCreateFlagFileEvents`, a rename surfaces as `kFSEventStreamEventFlagItemRenamed`, delivered as two events — one at the old path, one at the new. Correlating them needs the file ID, which `kFSEventStreamCreateFlagUseExtendedData` supplies via `kFSEventStreamEventExtendedFileIDKey`. So within a watched tree, a move is not merely detectable, it is attributable.
- **History across downtime.** Streams can start from a stored event ID rather than "now," and the volume's event log persists across reboots. So the agent can catch up on renames that happened while it was not running, which is the case that would otherwise force a full rescan.

What it does not give us, and these are the cases the durable identity exists for:

- **A folder moved out of the watched tree.** Rename the source folder itself and we get `kFSEventStreamEventFlagRootChanged` — we learn the root moved, not where it went. Finding it again means resolving by file ID or bookmark, which is precisely the mechanism FSEvents was supposed to replace.
...
- **iOS.** There is no FSEvents. Bookmarks are the only option there, so the durable identity column is required regardless of what the Mac can do.

The conclusion is that they compose rather than compete: **FSEvents is the fast path, durable identity is the recovery path.** Watching gives immediate, cheap, attributable rename handling for the common case; the stored identity is what answers "where did this go" when we were not watching, were watching the wrong tree, or are on a platform without the luxury. Which means the neat trick still has a job — just a smaller and less frequently exercised one than it would have had.
...
It lives in `UserDefaults` as `repeatWindowFraction`, settable by `pgr set` from day one and worth a slider in the settings GUI later, since it is the one parameter whose effect a person can actually feel.
...
Every surface deals from this one sequence, and the processes are separate, so an in-process lock is worthless. The primitive is a write transaction that selects and updates in one statement:
...
Every display is a separate virtual consumer, and each consumer deals a *hand*: a contiguous block of *n* cards reserved from the shared deck in one operation. Display one takes cards 1…n, display two takes n+1…2n, the screensaver takes the next block after that. Reservation is atomic, so hands are disjoint whenever the deck has enough cards to make them disjoint.

When it does not, they overlap rather than starve. A library of one photo and three displays means all three show that photo; a library of twelve photos and a screensaver wanting a hundred-card hand means the screensaver gets twelve and cycles. The reservation query takes what it can and returns short, and a short hand is a normal result rather than an error — the consumer simply reserves again sooner. This is the same degradation rule as the repeat window, and it exists for the same reason: every path that produces a photo must always produce a photo, because the alternative is a black screen.

```sql
CREATE TABLE consumer (
  id         INTEGER PRIMARY KEY,
  kind       TEXT NOT NULL,      -- 'wallpaper' | 'screensaver' | 'widget'
...
CREATE TABLE hand (
  ...
  position    INTEGER NOT NULL,  -- order within the hand
  played_at   INTEGER,
  PRIMARY KEY (consumer_id, position)
);
```

This turns out to solve three problems at once, which is usually a sign the abstraction is the right one.

**It fixes the screensaver's write access.** Earlier in this plan, the shared-deck decision meant the saver had to reach the canonical database every ten seconds — a hard requirement to place on code running inside someone else's sandbox. With hands, the saver reserves a hand of a hundred cards, plays through it locally for sixteen minutes, and reserves again. One write every sixteen minutes is a dramatically easier thing to arrange, and the consumption-journal fallback becomes entirely comfortable at that rate rather than marginal.

**It bounds the cross-process contention.** Dealing one card at a time from three surfaces means constant `BEGIN IMMEDIATE` contention on a single-writer database. Reserving hands moves that to a handful of writes an hour.

**It gives the prefetcher something concrete to work on.** The cache no longer has to guess what is needed soon — reserved hands *are* the answer, explicitly. The materialization queue is the union of unplayed cards across all outstanding hands, in position order, and the "never evict inside the deal horizon" guard becomes "never evict a card in an outstanding hand."

Details that matter:

- **Hand size is per-consumer.** A wallpaper at one photo per thirty minutes wants a small hand — perhaps 4, covering two hours — because a large one would reserve cards it will not show for days, keeping them out of everyone else's reach and pinned in the cache. A screensaver wants a large one. The default should be derived from the consumer's known interval: enough cards to cover roughly twenty minutes of playback.
- **Unplayed cards must be returnable.** A display that gets unplugged, or a screensaver that exits after two minutes of a hundred-card hand, is holding 98 cards nobody will see. Those must return to the deck rather than being silently consumed, or the library visibly thins out over weeks. The rule: a card is dealt — deal ordinal assigned, window clock started — when it is *played*, not when it is reserved. Reservation only marks it as spoken for. An abandoned hand is reclaimed by a reaper in the agent, keyed off the consumer's `seen_at` heartbeat.
- **Displays need stable identity.** A consumer is tied to a display so the same monitor resumes its own rotation after a sleep or a cable swap. `CGDisplayCreateUUIDFromDisplayID` gives a UUID that survives reboots and port changes, which is the right key; the transient `CGDirectDisplayID` is not.
- **Widgets are consumers too, but degenerate ones.** A widget timeline provider reserves a hand covering the entries it is about to generate, which is a natural fit — WidgetKit already asks for several future entries at once.

## Consequences of one shared deck

Every surface — wallpaper, screensaver, each widget — deals from this one sequence, and the repeat window applies across all of them together. Three things follow, and all three are real work rather than footnotes.

**Dealing has to be atomic across processes**, which is why the deal is a single `BEGIN IMMEDIATE` transaction with `UPDATE … RETURNING` rather than a select followed by an update. Detailed under *The deck algorithm* above.

**The cache has to keep up with the screensaver, not the wallpaper.** A wallpaper at one photo per half hour needs 48 photos a day. A screensaver at one per ten seconds needs 360 an hour. Because they share a deck, the screensaver sets the pace for everything: the prefetch window has to be deep enough that an evening of screensaver never catches up to the edge of the cache, and the eviction policy has to tolerate the deck advancing hundreds of positions in an hour. Concretely, this is what the 1000-photo cap has to be validated against: a cap is big enough when a long screensaver session never outruns it. The outstanding hands make the requirement precise — every unplayed card in every hand must be resident, so the cap has a hard floor at the sum of all hand sizes.

It also means a long screensaver session can roll the entire library over. With a 4,000-photo pool and a six-hour session, you finish the pass and start the next one. That is correct behavior for a shared deck, but it means the wallpaper's "every photo before any repeat" guarantee is measured against a deck the screensaver is consuming, so in practice the wallpaper sees a near-random sample rather than a slow walk through the library. Worth knowing before it looks like a bug.

**The screensaver must be able to advance the deck, which raises the bar for the Phase 6 spike.** With per-surface decks, a read-only screensaver could have kept its own position in its own file. Sharing the deck means the saver's ten-second tick has to reach the canonical database. Read-only access to a copied cache directory is no longer sufficient.

The mitigation, if the sandbox blocks direct database writes, is a consumption journal: the saver appends dealt-photo records to a file inside its own container, and the agent watches that file and applies the advances to the real deck. This is eventually-consistent rather than atomic — the saver has to pick its next photo from a locally-cached window rather than dealing live — so the window and the journal need to be sized so the agent can keep the window refilled. It works, but it is meaningfully more machinery than a direct write, which is why the spike should test direct database access from inside `legacyScreenSaver` first and treat the journal as the fallback.
...
An earlier draft of this plan justified stored derivatives by claiming a large decode would stutter the screensaver. That does not survive the hands design: the next card is known up to sixteen minutes in advance, so its decode happens on a background thread with enormous lead time. Even RAW, the slow case, has orders of magnitude more slack than it needs.
...
**One measurement would close the remaining doubt,** and it belongs in Phase 2 as a `pgr` subcommand: subsampled decode time to display size for the worst files in a real library — a large ProRAW, a 48-megapixel HEIC, a stitched panorama — on this machine. If those land in the tens of milliseconds, as expected, the case for on-disk derivatives disappears entirely. If a panorama exceeds the GPU's maximum texture dimension, that is a separate problem needing tiling rather than a derivative file.

### Chunked ingestion

Adding a source does three things, in this order, and the ordering is the whole user experience:

1. **Enumerate and insert all rows.** For a large album this is seconds, and it is one batched write — insert in transactions of about 500 rows rather than 50,000 individual statements or one enormous transaction that holds the write lock for a minute. New rows have a null `last_dealt_seq` and are eligible at once. At the end of this step the deck is fully ordered even though almost nothing is downloaded.
2. **Materialize the first chunk of 10, at elevated priority.** Ten photos is enough to start the wallpaper rotating and to give a screensaver somewhere to begin. This is the "burst" — it should complete in seconds for local files and in however long ten iCloud downloads take otherwise.
3. **Materialize the rest in chunks of 10, at background priority,** until the cache cap is reached.

Chunking at 10 rather than fetching continuously matters for several reasons at once: it bounds peak memory to ten in-flight images rather than an unbounded queue; it gives natural checkpoints to re-read configuration, re-check thermal state, and notice that the user disabled a source; it keeps a `BGProcessingTask` on iOS able to stop cleanly at a chunk boundary when its time expires; and it avoids issuing 50,000 concurrent PhotoKit requests, which is a good way to make Photos stop answering. The chunk size is configurable for the same reason the cap is — it is an empirical number, not a principled one.

### Eviction

FIFO by materialization time, capped at 1000 photos by default.

The reason plain FIFO is correct here rather than something cleverer is that materialization happens *in deck order*. The prefetcher always works on the next un-materialized photo in deck order, so the order photos enter the cache is the order they will be dealt. Oldest-added is therefore also longest-since-dealt, and evicting from the front of the cache is evicting the photo the deck is furthest from reaching again. The cache is a sliding window over the deck: expire off the front, fill at the back, exactly the deck metaphor from the original sketch.

Two guards on top of it:

- **Never evict a photo inside the deal horizon.** If a photo is among the next few the deck will hand out, it stays regardless of age. Without this, a screensaver dealing fast enough could evict a photo moments before requesting it.
...
The 1000 default is explicitly a starting point to be replaced by measurement. The experiment belongs in Phase 2, as a shell loop over `pgr` against real sources, and it should record, for cache caps of roughly 250 / 500 / 1000 / 2000 / 4000:
...
- steady-state agent memory and CPU while prefetching;
...
The same policy with much smaller numbers and a different filling schedule. A phone should not carry 1000 originals, so iOS caps lower. Filling happens in the foreground and in `BGProcessingTask` windows when charging, in the same chunks of 10, which is exactly why the chunk boundary is a clean stopping point.

## The cold-start burst

Your "at least 10 photos as quickly as possible, then backfill to 1000" is really two separate paths, and conflating them is how this ends up feeling slow.

**Warm start** — cache populated, deck rows present. Reading the top of the deck is a single indexed query returning file paths that already exist. Sub-millisecond. Every launch after the first should be this.

**Cold start** — no cache, or a new source just added. The agent enumerates the source's identifiers first (fast: `PHAsset.fetchAssets` returns lazily, a directory walk is I/O-bound but cheap), inserts all rows with random shuffle keys and no deal ordinal, and only then starts fetching bytes. That means the deck is fully *ordered* within seconds even if the photos are not yet local. It then fetches the first chunk of 10 at elevated priority, signals readiness, and drops to background priority for the remaining chunks until the cache cap is reached.

The display layers need a "not ready yet" state for the genuinely-cold case: for wallpaper, leave the existing one; for the screensaver, hold on the current image rather than showing a gap.
...
The one thing to get right is that an evicted orphan must not be re-materialized. The prefetcher skips photos whose source is unavailable, so it never tries and never logs a storm of failures.
...
- **`pgr cache clear`**, with `--source <id>` to drop one source's bytes, `--unavailable` to drop everything belonging to sources that are gone, and no argument to empty it entirely.
...
**Deck history survives; only bytes are discarded.** Epochs, shuffle keys, and last-shown times are untouched, so a cleared cache refills into the same rotation rather than reshuffling the library. Clearing is a storage operation, never a shuffle operation.

**Recovery is the cold-start path, which already exists.** Outstanding hands are returned to the deck, since their cards no longer have bytes behind them. The prefetcher then does exactly what it does for a new source: first chunk of ten at elevated priority, then background chunks to the cap. Displays hold their currently decoded image — which is in memory, not in the cache — until the first new card is ready, so clearing the cache does not blank a screensaver mid-session.
...
- **`pgr source list`** marks it inline, with the reason and the time it went unavailable.
...
The obvious value is that the server becomes demonstrable on its own — sources can be added, a library scanned, a deck dealt, and a cache filled, all before a single view exists. The less obvious value is that it makes the shuffle *testable in the way shuffles actually need to be tested*, which is statistically. "Does this feel random?" is not a question a GUI can answer. "Deal fifty thousand cards across four thousand photos and assert every pass contains every photo exactly once, then report the distribution of gaps between consecutive showings of the same photo" is a question a command-line tool answers in a script, in a second, repeatably.

A rough shape of the subcommands:

```
pgr source add --folder <path> | --file <path>
pgr source list | remove <id> | enable <id> | disable <id>
pgr scan [--source <id>]
...
- **It doubles as the measurement rig.** The cache-cap experiment — fill times and cache-miss rates across caps of 250 through 4000 — is a shell loop over `pgr` invocations, not a bespoke benchmark harness. Same for validating that hand reservation actually serializes: run several `pgr deck deal` processes concurrently and assert the union of their hands has no duplicates.

Between them, `pgr` and the Mac app cover the two halves of the problem: the command line for anything scriptable, statistical, or repeatable, and the app for anything visual, interactive, or timing-dependent.

## The Mac app as instrument panel

**In Phase 3 the app is deliberately almost nothing.** A window, a photo in it sized to fit, the pan, a timer, and standard full-screen support — the green button and `toggleFullScreen:`, no bespoke presentation layer. It manages no sources and exposes no settings, because `pgr` shipped one phase earlier and already does both. That keeps the milestone small and keeps the app honest: it is a *consumer*, and consumers display cards.
...
- **Consumer simulation.** Spin up two, three, five virtual consumers at configurable rates and watch hands get reserved and played. This is how the shared-deck contention story gets tested — the "screensaver at one photo per ten seconds starves the wallpaper" scenario is a slider in this app long before there is a screensaver.
...
- **`OSSignposter` for intervals**, not log lines: decode-to-display time, materialization duration, scan duration, hand reservation latency. These are the numbers the Phase 2 measurements need, and signposts make them readable in Instruments without building a benchmark harness.
- **`pgr log`** wraps `log show --predicate 'subsystem == "com.sydpolk.photogoround"'` with sensible defaults and a `--follow` mode, because nobody should have to remember predicate syntax to see what the server is doing.
...
`PhotoGoRoundKit` is where the logic lives, so that is where the tests live. The deck algorithm is pure and testable against an in-memory database: at fraction 1.0, assert that a thousand deals across a hundred photos produce exactly ten showings each with no repeat inside any hundred-deal stretch; at lower fractions, assert no repeat inside the window and report the gap distribution. The cache manager is testable with a fake provider yielding synthetic files: assert that FIFO eviction holds the cap, that nothing inside an outstanding hand is ever evicted, and that an abandoned hand's cards return to the deck. The source providers get integration tests behind a protocol so folder sources test for real and Photos tests against a fixture library.

Above the unit tests sit three layers, each catching what the one below cannot:

- **`pgr`** for anything scriptable, statistical, or concurrent — shuffle distribution over fifty thousand deals, cache-cap sweeps, and several processes reserving hands at once to prove the reservation actually serializes. These are assertions a test suite can run in CI as easily as a person can run them by hand.
...
**The fallback if nothing works: a consumption journal.** The saver appends dealt-card records to a file in its own container, and the agent watches that file and applies the advances to the real deck. Eventually consistent rather than atomic, so the saver plays from a locally-cached hand rather than dealing live — which the hand design already accommodates, since the saver only needs to reach the database once every sixteen minutes rather than every ten seconds.
...
**Preview mode must not consume the deck.** `ScreenSaverView` is instantiated with `isPreview: true` for the thumbnail in System Settings. That instance must not reserve a hand — if it did, idly browsing screensaver settings would deal cards that nobody ever sees, and with a shared deck those photos are then spent for the wallpaper too. Preview reads a fixed sample of already-cached photos and advances nothing.

**One instance per display, and each is its own consumer.** macOS creates a separate `ScreenSaverView` for every attached screen, which lines up exactly with the consumer model: each gets its own hand, so the displays show different photos, and each computes its own pan against its own aspect ratio.
...
- **Execution model.** The Mac agent is a long-lived process with a timer that can fetch continuously for an hour. The iOS widget is invoked, gets a few hundred milliseconds to produce a `TimelineEntry`, and dies. There is no shared abstraction over those two things that is not a lie, so the kit exposes work as discrete callable units — `advanceDeck()`, `prefetchNext(budget:)`, `evictIfNeeded()` — and never schedules anything itself.
...
Preferences live in the App Group's `UserDefaults` suite; the database holds state. `pgr set` is the blessed way to change a preference because it knows the right domain and posts the change notification afterwards. But raw `defaults write` must work too, from any terminal, with no cooperation — that was an original requirement and it is the harder half.
...
- **Watch the preferences plist's containing directory,** not the file. `defaults` writes through `cfprefsd`, which replaces the plist atomically via rename. A `DispatchSource.makeFileSystemObjectSource` on a file descriptor is therefore invalidated by the very event we care about — the descriptor now points at an unlinked inode. Watch the directory with an FSEvents stream, or re-arm the vnode source on every `.rename` and `.delete`. This is the single most common way to implement this wrong and have it work in testing, then silently stop after the first write.
- **Force a re-read before reading.** `cfprefsd` caches aggressively, and a stale value in our process is the default outcome. Call `CFPreferencesAppSynchronize` for the domain — or `UserDefaults.synchronize()`, deprecated but still the mechanism — before pulling values, or we will faithfully notice a change and then read the old number.
- **Poll slowly as a backstop.** `cfprefsd` batches writes, so a file event can lag by seconds, and a missed event should not mean a preference that never takes effect. A re-read every thirty seconds costs nothing and turns "broken" into "slightly delayed" in every case the watcher misses.
- **Then broadcast.** Having noticed, the server posts the Darwin notification so the app, the widget, and the saver re-read too. The server is the reconciler: external writes come in through it, and everything else learns about them the same way it learns about `pgr set`.
...
| Cache cap | Run an eviction pass now if lowered; otherwise let the next prefetch fill to the new cap |
| Chunk size | Next chunk boundary — which is one reason chunks exist |
| Repeat window fraction | Next deal; recomputed from the current pool size, no state to rebuild |
| Hand size | Next reservation. Outstanding hands are left alone rather than resized, since their cards are already materialized |
...
- The consumer and hand API — a new surface is a new consumer row, not a new code path in the deck.
...
Adding this to the entitlements ledger: the server carries `NSPhotoLibraryUsageDescription` and needs Files-and-Folders consent; the app and `pgr` ideally carry neither, because they never touch either resource.
...
- **A Darwin notification still does the waking.** `UserDefaults` change observation is unreliable across process boundaries, so a writer posts a notification and readers re-read. `pgr` posts it after any write, so `defaults write` followed by `pgr notify prefs` takes effect immediately rather than at the next poll.
- **The screensaver's preference domain is not where you think it is.** A `.saver` running sandboxed inside `legacyScreenSaver` reads preferences from that host's container, not from `~/Library/Preferences/`. `ScreenSaverDefaults(forModuleWithName:)` exists to handle exactly this, and it means a plain `defaults write` from the terminal will appear to do nothing to screensaver settings until it is aimed at the right domain. Worth a `pgr` subcommand that writes to the correct place so the right path is discovered once rather than every time.
