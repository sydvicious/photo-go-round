...
- **Phase 3** — **The Mac app that calls it.** Just a window showing the shuffle — sized to fit, with the pan — that can be taken full screen. No source management, no settings: `pgr_ctl` already does that. This is the milestone that proves the whole idea, and it is small. **As built, 2026-09-12**: the window and full screen, a Settings panel for sources and the wallpaper, and the About box's dashboard link; the pan waits with the saver's motion, and watching and the fit options are not built.
  ...
  - **The agent performance overhaul — in progress since 2026-09-16, in its own plan, `Agent Performance Overhaul.md`.** The app and the screensaver lost the agent three times in one afternoon; a thread sample showed every thread in the shared pool stuck resizing and waiting on the write lock. Built so far: the `TIMING:` and `LOCK:` log lines, a one-second budget on serving's *is it still there?* check, one serial `Resizer` off the pool, and a sized request that sends the original when its resize takes over a second. Still to come: the resize cache returning, a thread per request, the refresh locking only to write in pages of 100, and refresh and downloads on actors of their own.
...
- **Every surface has a defined empty state.** The screensaver bounces "No Photos Available" around the screen; the wallpaper leaves the existing desktop alone; the widget shows a static label. Nothing anywhere renders a blank screen that could be mistaken for a crash.
...
- **The database holds state; `UserDefaults` holds preferences — with one named exception.** Sources, pool, queue, and cache are state. Fits, timings, transitions, and caps are preferences, living in the `com.sydpolk.photogoround` preference domain — `.dev` in development — so they are settable from the command line. The exception is `servicePort`, which the agent publishes because a client needs it to find the service at all and there is no other way to be told. Everything else the agent derives is answered over HTTP rather than written down.
...
- **Materialized.** Everything else, copied into the cache root — `~/Library/Caches/com.sydpolk.photogoround` with `--prod`, `<repo>/.build/pgr-cache` in development. These are what the ceiling governs.
...
**One resize at a time, off the shared pool, and never more than a second's wait. Since 2026-09-16.** Resizing ran on Swift's shared thread pool, and on 2026-09-16 six resizes at once held it while the system's HEIC decoder was slow — single resizes took up to 741 s and requests that had nothing to do with resizing could not start. A probe on Syd's originals found nothing needs the main thread and HEIC stops getting faster past two at once. Every resize now runs on one serial `Resizer` actor with its own dispatch queue; a request waits at most `ServiceTiming.resizeBudget`, one second, and then streams the original, and a resize still queued when its request gives up is skipped. Syd, choosing between moving resizing into every client and keeping it here: "the agent will continue to do the resizing; the clients don't need to mess with this complication." `Agent Performance Overhaul.md`, *The resize probe* and *When the resizer stalls*.
...
- **A control in the settings UI**, showing bytes held per source with a way to reclaim them. **Not built, deliberately for now**: the Settings panel exists, and cache clearing stays in `pgr_ctl` with every other verb the app did not take on — see `app/mac/FEATURES.md`.
...
**Being reversed, decided 2026-09-16.** Syd, once the agent was resizing one picture at a time: "we should start caching the resized images again to reduce the workload on the resize queue", and "We will pretty much resize each picture once if there is a small number of pics in the sources and everything fits in the cache, and save the expensive HEIC resizer resource." Eviction: "based on LRU of the files themselves, and don't bother with removing resized images if the original is evicted", then "strictly based on creation date of the file, or an equivalent semantic in the database." Not designed yet; `Agent Performance Overhaul.md`, Phase 2b. What follows is the 2026-09-06 decision, kept for its reasoning — the drifting-box problem it describes still has to be answered by the new design.
...
- `fitting:` through `serve`, `bytesHere`, `firstQueuedWithBytes`, `waitForBytes` — the box existed so an evicted original could be answered from a rendering at exactly that size. `bytesHere` now only calls `residentURL`, which still resolves a referenced file in place.
...
The property that actually matters is untouched: **configuring this library never requires the agent to be up**, because `pgr_ctl` writes preferences directly. The rig covers the cold case; a client covers the case where there is a running system to talk to. The app is to be what registers the agent — not built yet; see `app/mac/FEATURES.md`, *The app brings its own agent* — so a user who has one will have the other.
...
- **The source is named beside its row id**, `source 6 (Photos › Trips › Holiday)`, from `Source.spokenName` in `PhotoGoRoundAgentAPI` so the agent, the page, and a client all say the same thing. The dashboard takes the source's name at the moment of serving, so a source removed since keeps the name it was served under.
...
**It does manage sources, and that is a reversal of what this section said.** The original text — "it manages no sources and exposes no settings, because `pgr_ctl` shipped one phase earlier and already does both" — did not weigh that `pgr_ctl` never ships, which makes it no answer at all for anybody who is not the author. A Settings panel for sources arrives here; every other verb stays in the rig. See `app/mac/FEATURES.md`. Since then Settings has gained the *Also set wallpapers* checkbox, and the About box links the agent's dashboard. *The checkbox went on 2026-09-16 with the app's wallpaper loop; Settings keeps the wallpaper's *Shuffle All* pop-up, which times the wallpaper extension.*
...
**Of that list, only the fit and the black bars were built.** `AspectFit` is in `PhotoGoRoundDisplay` and the window uses it; `Pan` is in there beside it, tested, with no callers anywhere in the project; the cross-fade was never started, and the bouncing empty state waited until 2026-09-09, when it was built in `PhotoGoRoundDisplay` for the window and the saver together. The rehearsal did not happen, and Phase 6 arrived at the sandbox with the visual design still ahead of it rather than behind it. **The argument above is not wrong and is worth keeping** — it is why the 2026-09-07 decision splits the saver's v1 into the picture now and the motion later, in the window, where this section always said it belonged.
...
The server — and the backend half of the iOS app — logs structurally, through Apple's unified logging and nothing else. `OSLog`/`Logger`, subsystem `com.sydpolk.photogoround`, one category per subsystem: `sql`, `deck`, `cache`, `sources`, `photos`, `prefs`, `wallpaper`, `saver`, `widget`, with signposts under `intervals`. `Log.saver` and `Log.widget` are declared and unused: the saver declares a `Logger` of its own, and there is no widget yet.
...
**Planned in `Wallpaper Plan.md`, 2026-09-10, which settles several points in this section.** The wallpaper is a client — the Mac app runs it first, then a binary of its own — so wherever this section says *the agent* re-applies or reasserts, read *whatever runs the wallpaper*. The background colour is not ours to set: the space around the photograph is the fill colour chosen in System Settings. The files the desktop points at live in `~/Library/Application Support/com.sydpolk.photogoround.wallpaper.{dev|prod}/`, never in the agent's container — "the app should not need to see the agent's container." And it runs only while the app's *Also set wallpapers* checkbox is ticked. **As built, 2026-09-12**, the app's wallpaper always uses the development deployment — the `.dev` domain and directory, and the development agent — because the app asks for `.development` throughout until *The app brings its own agent* decides otherwise. **Superseded 2026-09-16:** the app's loop is removed; the wallpaper is an ExtensionKit extension on `com.apple.wallpaper`, chosen in System Settings, which draws into surfaces `WallpaperAgent` hands it rather than setting files — so `setDesktopImageURL`, the files, the per-Space hole and the reverting all belong to the removed loop. `Wallpaper Plan.md`, *The real extension, inside the app* and *The app's loop, removed*.
...
| Cache byte ceiling | The next maintenance pass evicts to it; raising it simply lets the cache hold more |
...
**Which domain, which is the part that will waste an afternoon if not written down — corrected 2026-09-12, because as built it is simpler than this section planned.** It planned three: an App Group suite, the screensaver's own `ScreenSaverDefaults` domain, and a stray `com.sydpolk.photogoround` nothing read. As built, the agent's settings and the source list live in `com.sydpolk.photogoround` itself — `com.sydpolk.photogoround.dev` in development — in `~/Library/Preferences/`, which is exactly what a person types; there is no App Group suite and nothing watches a second domain. The screensaver has no settings of its own, and reads the agent's plist file directly for the port, because its sandbox hands it an empty suite — see `Screensaver Plan.md`. **2026-09-10:** the wallpaper adds domains of its own, `com.sydpolk.photogoround.wallpaper.{dev|prod}`, holding its interval (`intervalSeconds`), each display's change time and file, and its *Also set wallpapers* setting — nothing the server reads. *2026-09-16: those domains hold one key, `interval`, a *Shuffle All* tag; the rest went with the app's loop.*
...
| --- | --- | --- |
| Mac agent | No | `NSPhotoLibraryUsageDescription`. **As built, 2026-09-12**: no entitlements file, no App Group, and no hardened runtime yet; Developer ID and notarization are the 1.0 target |
| Mac config app | No | Same; ~~registers the agent via `SMAppService`~~ installs a per-user LaunchAgent plist in `~/Library/LaunchAgents` (2026-09-10; decided, not built). Probably ships as a menu-bar app rather than the full window — TODO.md, *A menu-bar app for shipping* |
...
4. ~~**No Xcode project until Phase 3.**~~ **Done: every product is a target in `app/Photo-Go-Round.xcodeproj` since 2026-09-08**, and the App Group container question it was waiting on went away instead of being settled. See *The agent: registration and permissions*.
...
- **The agent** registers itself on first launch via `SMAppService`. Nothing to install. **Corrected 2026-09-10:** the app installs the user's plist in `~/Library/LaunchAgents` on first launch, as above.
