...
- *The app brings its own agent* — installing Photos-Go-Round should be the whole of installing Photos-Go-Round. **Not built.**
  ...
  - Decide which deployment the embedded agent runs in. **Decided 2026-09-23:** the build's own — production in Release, development in Debug and Claude. The app, the screensaver, the wallpaper extension and the agent's plist all follow it; a Release plist passes `--prod`. See `Deployment.current`.
