...
- *Saying the agent is not there* — **done.** The window title gains the trouble's words — `Photos-Go-Round - Waiting for Photos` since 2026-09-16 (`Photo-Go-Round - …` until the rename of 2026-09-22), `Photo-Go-Round - Photo-Go-Round Is Not Running` from 2026-09-09, and `Photo-Go-Round - No agent` before that — and the photograph dims behind a flat grey at three-tenths.
...
- *The empty state moves* — when there is nothing to show, say so without leaving a still frame on the glass. **Done 2026-09-09**, except the refusal message below, which waits for *The app brings its own agent*.
  ...
  - **One exception, and it is narrow**: when registering the agent was *refused*, the window says "Problem launching the agent. Contact support@sydpolk.com." — see *The app brings its own agent*. Anything else with no agent, including one that registered and later stopped, is "Waiting for Photos".
...
- *The app brings its own agent* — installing Photos-Go-Round should be the whole of installing Photos-Go-Round. **Not built.**
  ...
  - Decide which deployment the embedded agent runs in. **Decided 2026-09-23:** the build's own — production in Release, development in Debug and Claude. The app, the screensaver, the wallpaper extension and the agent's plist all follow it; a Release plist passes `--prod`. **Since 2026-09-24 the agent follows it too**, however it is started — Syd: "a release build should always install and use a release agent, period, no matter how it is launched" — so the plist's `--prod` is only a statement of what it runs. See `Deployment.current`.
...
- *The agent's dashboard, from the About box* — **built 2026-09-12.** Syd, 2026-09-12: "the reason I want it in the about box is that gives me the port number. it should open the dashboard in the system browser, not a webview in the app."
  ...
  - With no port published it says "Waiting for Photos", the picture window's words for the same condition. A preference domain that cannot be read says so, with the reason.
...
- **The screensaver's domain** is `com.sydpolk.photosgoround.screensaver.dev`, or `.screensaver.prod` in production, beside the wallpaper's `.wallpaper.dev` and `.wallpaper.prod`.
