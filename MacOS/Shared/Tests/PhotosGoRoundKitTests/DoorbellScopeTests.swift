...
struct DoorbellScopeTests {
    ...
    func differentDatabasesRingDifferently() {
        let agent = doorbells("/Users/someone/Library/Containers/pgr/photosgoround.sqlite")
        let scratch = doorbells("/tmp/test-9f3a/photosgoround.sqlite")
    ...
    func theSameDatabaseAgrees() {
        ...
        let one = doorbells("/var/pgr/photosgoround.sqlite")
        let two = doorbells("/var/pgr/photosgoround.sqlite")
        #expect(one == two)
        #expect(one.name(.sourcesChanged) == two.name(.sourcesChanged))
        #expect(one.namespace == DarwinNotification.Doorbells.namespace(for: "/var/pgr/photosgoround.sqlite"))
    }

    @Test("A path is standardized first, so two spellings are one library")
    func spellingDoesNotMatter() {
        #expect(doorbells("/var/pgr/../pgr/photosgoround.sqlite") == doorbells("/var/pgr/photosgoround.sqlite"))
    }
    ...
    func topicsStayReadable() {
        ...
        let bells = doorbells("/var/pgr/photosgoround.sqlite")
        #expect(bells.name(.sourcesChanged).hasPrefix("com.sydpolk.photosgoround."))
